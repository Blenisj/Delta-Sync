...
def save_telemetry(lap_num, final_time=None):
    ...
    try:
        ...
        metadata["driver_name"] = driver_name # <--- Added
...
def acMain(ac_version):
    global appWindow, speed_label, car_name, track_name, driver_name, session_id, current_lap_index, session_best_lap
    ...
    try:
        ...
        # --- SAFE NAME GRAB ---
        # No sandbox-breaking OS calls. Tries to get Steam name, defaults to Local Driver.
        raw_name = ac.getDriverName(0)
        if raw_name == -1 or raw_name is None:
...
def acUpdate(deltaT):
    ...
    try:
        ...
        lap_progress = ac.getCarState(0, acsys.CS.NormalizedSplinePosition)
    except Exception as e:
        ac.log("DeltaSync Read Error: {}".format(e))
        return 
    ...
    # TRIGGERED WHEN CROSSING THE FINISH LINE
    if actual_lap > current_lap_index:
        ...
        if finished_lap_time > 0 and current_lap_index > 0: 
    ...
    if current_lap_index > 0:
        ...
        current_lap_time_ms = int((time.time() - lap_start_time) * 1000)
...
def acShutdown(*args):
    """Fires automatically when the user exits the session."""
    try:
        base_dir = os.path.dirname(__file__)
        flag_path = os.path.join(base_dir, "kill_uploader.flag")
        
        with open(flag_path, "w") as f:
            f.write("die")
        ac.log("DeltaSync: Dropped kill flag for uploader.")
    except Exception as e:
        ac.log("DeltaSync: Failed to drop kill flag. Error: {}".format(e))